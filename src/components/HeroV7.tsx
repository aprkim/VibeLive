...
/**
 * HeroV7 - Polished hero with improved design
 *
 * Improvements:
 * - Subtle gradient mesh background
 * - Syntax-highlighted code snippets at low opacity (IDE-like)
 * - Code window lifted with glow effect
 * - Better button hierarchy (ghost secondary button)
 * - Improved typography with better line-height
 * - Framework logos for visual trust
 */
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 0 }}>
        ...
        <div
          ...
          style={{
            background: `
              radial-gradient(ellipse 80% 50% at 50% -20%, rgba(14, 165, 164, 0.08) 0%, transparent 50%),
              radial-gradient(ellipse 60% 40% at 80% 60%, rgba(96, 165, 250, 0.05) 0%, transparent 40%),
              radial-gradient(ellipse 60% 40% at 20% 80%, rgba(14, 165, 164, 0.04) 0%, transparent 40%)
            `
          }}
        />
        ...
        <div
          ...
          style={{
            width: '9px',
            height: '9px',
            backgroundColor: 'rgba(14, 165, 164, 0.45)',
            borderRadius: '50%',
            right: '18%',
            top: '18%',
          }}
        ...
        />
        {/* Bottom right - tiny teal */}
        <div
          className="presence-node-pulse-delay-1 absolute"
          style={{
            width: '5px',
            height: '5px',
            backgroundColor: 'rgba(14, 165, 164, 0.4)',
            borderRadius: '50%',
            right: '25%',
            bottom: '25%',
          }}
        ...
        {/* Subtle IDE-like code background */}
        <div className="absolute inset-0 flex flex-col justify-center items-center opacity-[0.04]">
          <pre className="font-mono text-[10px] md:text-xs text-text leading-relaxed whitespace-pre select-none">
{`import { VibeLive } from '@vibelive/sdk'
import { VideoRoom } from '@vibelive/react'

const App = () => {
  const { room, participants } = useVibeLive()
...
// No WebRTC configuration needed
// No STUN/TURN servers to manage
// No signaling infrastructure

export default VibeLive.init({
  apiKey: process.env.VIBELIVE_KEY,
  enabled: true
})`}
          </pre>
        </div>
        ...
        <div className="max-w-[980px] mx-auto">
          ...
          <div className="text-center mb-16">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-6">
              ...
              <button className="px-7 py-3.5 text-base w-full sm:w-auto font-medium text-accent border border-accent/30 rounded-[10px] hover:bg-accent/5 hover:border-accent/50 transition-all duration-200">
                How it works
              </button>
            </div>
            ...
            {/* Framework logos for trust */}
            <div className="flex items-center justify-center gap-6 opacity-40">
              <span className="text-xs text-muted font-medium">Works with</span>
              <div className="flex items-center gap-5">
                ...
                <svg className="w-5 h-5 text-muted" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20.68 2.016a7.596 7.596 0 00-10.088 2.694l-4.073 6.278a6.667 6.667 0 00-.967 4.713 6.24 6.24 0 001.106 2.617 6.013 6.013 0 00-.566 1.873 6.426 6.426 0 00.327 3.26 7.596 7.596 0 0010.088-2.694l4.073-6.278a6.667 6.667 0 00.967-4.713 6.24 6.24 0 00-1.106-2.617 6.013 6.013 0 00.566-1.873 6.426 6.426 0 00-.327-3.26zm-9.23 18.592a4.432 4.432 0 01-4.768-1.683 3.842 3.842 0 01-.623-2.56 3.453 3.453 0 01.132-.752l.092-.27.24.164a7.104 7.104 0 002.136 1.106l.203.062-.019.205a1.16 1.16 0 00.225.848 1.33 1.33 0 001.431.505 1.219 1.219 0 00.351-.143l4.072-6.278a1.076 1.076 0 00.176-.756 1.33 1.33 0 00-1.431-.505 1.219 1.219 0 00-.35.143l-1.554 2.393a4.044 4.044 0 01-1.167.472 4.432 4.432 0 01-4.768-1.683 3.842 3.842 0 01-.623-2.56 3.583 3.583 0 01.587-1.588l4.072-6.278a4.044 4.044 0 011.167-.472 4.432 4.432 0 014.768 1.683 3.842 3.842 0 01.623 2.56 3.464 3.464 0 01-.132.753l-.092.27-.24-.165a7.104 7.104 0 00-2.136-1.106l-.203-.062.019-.205a1.16 1.16 0 00-.225-.848 1.33 1.33 0 00-1.431-.505 1.219 1.219 0 00-.351.143L7.4 13.238a1.076 1.076 0 00-.176.756 1.33 1.33 0 001.431.505 1.219 1.219 0 00.35-.143l1.554-2.393a4.044 4.044 0 011.167-.472 4.432 4.432 0 014.768 1.683 3.842 3.842 0 01.623 2.56 3.583 3.583 0 01-.587 1.588l-4.072 6.278a4.044 4.044 0 01-1.167.472z"/>
                </svg>
              </div>
            </div>
          </div>
      ...
      {/* Code window - lifted with glow */}
      <div className="relative z-20 px-6 -mb-20">
        <div className="max-w-lg mx-auto">
          <div className="rounded-[14px] overflow-hidden border border-border/80 bg-[#1e1e1e] shadow-2xl shadow-black/20 ring-1 ring-white/5">
            {/* Glow effect */}
            <div className="absolute -inset-4 bg-accent/5 blur-3xl rounded-full pointer-events-none" />

            {/* Editor Title Bar */}
            <div className="relative flex items-center gap-2 px-4 py-2.5 bg-[#252526] border-b border-[#3c3c3c]">
              <div className="flex gap-1.5">
                <div className="w-3 h-3 rounded-full bg-[#ff5f57]"></div>
                <div className="w-3 h-3 rounded-full bg-[#febc2e]"></div>
                <div className="w-3 h-3 rounded-full bg-[#28c840]"></div>
              </div>
              <span className="text-[11px] text-[#808080] ml-2 font-medium">vibelive.config.ts</span>
            </div>

            {/* Code Content - syntax highlighted */}
            <div className="relative px-5 py-4 font-mono text-[13px] leading-7">
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">1</span>
              ...
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">2</span>
                <span></span>
              </div>
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">3</span>
              ...
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">4</span>
              ...
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">5</span>
              ...
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">6</span>
                <span><span className="text-[#cccccc]">&#125;)</span></span>
              </div>
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">7</span>
                <span></span>
              </div>
              <div className="flex">
                <span className="w-6 text-[#6e7681] text-right mr-4 select-none">8</span>
                <span className="text-[#6a9955]">// That&apos;s it. Video chat is now live.</span>
              </div>
            </div>
          </div>

          {/* Caption below code */}
          <p className="text-center text-muted/60 text-xs mt-4 mb-8">
            Drop in one file. Video just works.
          </p>
        </div>
